...
NSString * const FLCPU124C41FreelansimLink = @"http://freelansim.ru/freelancers/CPU124C41";

+(void)createFavorites:(BOOL) kunst{
    
    __block FLFreelancer *freelancer = [[FLFreelancer alloc] init];
    if(kunst)
        freelancer.profile = FLKunstFreelansimLink;
    else
        freelancer.profile = FLDannyFreelansimLink;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        
        [self loadFreelancer:freelancer completion:^(FLFreelancer *fr) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if(fr){
                    NSArray *results = [FLManagedFreelancer MR_findByAttribute:@"profile" withValue:freelancer.profile];
                    if([results count] == 0){
                        ...
                        [managedOne mapWithFreelancer:freelancer andImage:img];
